...
func installCC(label string, ccPkg []byte, orgs []*OrgInfo) error {
	...
	for _, org := range orgs {
		orgPeers, err := sdkIntegration.DiscoverLocalPeers(*org.OrgAdminClientContext, org.OrgPeerNum)
		if err != nil {
...
func getInstalledCCPackage(packageID string, org *OrgInfo) error {
	// use org1
	orgPeers, err := sdkIntegration.DiscoverLocalPeers(*org.OrgAdminClientContext, 1)
	if err != nil {
